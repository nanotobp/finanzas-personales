...
import { Activity, TrendingUp, TrendingDown, DollarSign, Target, PiggyBank, CreditCard, AlertTriangle, LogOut } from 'lucide-react'
...
export function FinancialHealth() {
  ...
  const { data: healthData, isLoading } = useQuery({
    queryKey: ['dashboard-stats', currentMonth], // Reutilizar la misma cache
    queryFn: async () => {
  ...
  const getHealthColor = (score: number) => {
    if (score >= 80) return { bg: 'from-green-600 to-green-500', text: 'text-green-500', label: 'Excelente' }
    ...
    return { bg: 'from-red-600 to-red-500', text: 'text-red-500', label: 'Necesita atención' }
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-50 to-white px-4 pt-6 pb-24">
      <div className="max-w-md mx-auto space-y-6">
        {/* Header */}
        <div>
          <h1 className="text-2xl font-bold mb-1">Salud Financiera</h1>
          <p className="text-slate-600 text-sm">Análisis de tu situación actual</p>
        </div>
        ...
        <div className={`bg-gradient-to-br ${healthColor.bg} rounded-2xl p-5 shadow-lg`}>
          <div className="flex items-center gap-4 mb-3">
            ...
            <div className="flex-1">
              <p className="text-white/90 text-xs mb-1">Puntuación de salud</p>
        ...
        <div className="bg-white rounded-2xl p-4 border shadow-sm">
          <div className="flex items-center justify-between mb-3">
            <div className="flex items-center gap-2">
              ...
              <span className="text-slate-700">Uso de presupuesto</span>
            </div>
            <span className="text-sm font-bold text-slate-900">{healthData?.budgetUsage.toFixed(0)}%</span>
          </div>
          <div className="w-full bg-slate-100 rounded-full h-2 overflow-hidden">
            <div 
              className={`h-full transition-all duration-1000 ${
                healthData && healthData.budgetUsage > 90 ? 'bg-rose-500' : 
                healthData && healthData.budgetUsage > 75 ? 'bg-amber-500' : 'bg-emerald-500'
              }`}
              style={{ width: `${Math.min(healthData?.budgetUsage || 0, 100)}%` }}
            />
          </div>
        </div>
        ...
        <Button
          ...
          variant="outline"
          className="w-full hover:bg-rose-500 hover:text-white hover:border-rose-500 transition-colors"
        >
